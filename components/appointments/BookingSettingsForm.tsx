...
export default function BookingSettingsForm({ shopId }: { shopId: string }) {
  ...
  return (
    <div className="bg-crm-surface border border-crm-border shadow-sm rounded-xl p-6 space-y-5">
      <h3 className="font-bold text-crm-text text-2xl md:text-3xl">📅 Online Booking Settings</h3>
      ...
      <label className="flex items-center justify-between p-4 bg-crm-surface rounded-lg cursor-pointer text-sm">
        <div>
          <p className="font-medium text-crm-text text-base md:text-lg">Enable Online Booking</p>
          <p className="text-crm-muted text-base md:text-lg">Allow clients to book through your public portal</p>
        </div>
      ...
      <label className="flex items-center justify-between p-4 bg-crm-surface rounded-lg cursor-pointer text-sm">
        <div>
          <p className="font-medium text-crm-text text-base md:text-lg">Auto-Confirm Bookings</p>
          <p className="text-crm-muted text-base md:text-lg">Automatically confirm new bookings without manual approval</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="text-crm-muted block mb-1 text-sm">Min Advance (hours)</label>
          ...
          <p className="text-crm-muted mt-1 text-base md:text-lg">Minimum hours before a booking</p>
        </div>
        <div>
          <label className="text-crm-muted block mb-1 text-sm">Max Advance (days)</label>
          <input type="number" min={1} max={365} value={settings.maxAdvanceDays} onChange={e => upd('maxAdvanceDays', +e.target.value)}
            className="w-full bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm focus:outline-none focus:border-brand-gold" />
          <p className="text-crm-muted mt-1 text-base md:text-lg">How far ahead clients can book</p>
        </div>
        <div>
          <label className="text-crm-muted block mb-1 text-sm">Buffer Between Appts (mins)</label>
          <input type="number" min={0} max={60} step={5} value={settings.bufferBetweenAppointments} onChange={e => upd('bufferBetweenAppointments', +e.target.value)}
            className="w-full bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm focus:outline-none focus:border-brand-gold" />
          <p className="text-crm-muted mt-1 text-base md:text-lg">Cleaning / prep time between bookings</p>
        </div>
      </div>

      <div>
        <label className="text-crm-muted block mb-1 text-sm">Cancellation Window (hours)</label>
        <input type="number" min={0} max={168} value={settings.cancellationWindowHours} onChange={e => upd('cancellationWindowHours', +e.target.value)}
          className="w-full md:w-48 bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm focus:outline-none focus:border-brand-gold" />
        <p className="text-crm-muted mt-1 text-base md:text-lg">Clients cannot cancel within this window before their appointment</p>
      </div>

      <div>
        <label className="text-crm-muted block mb-1 text-sm">Cancellation Policy Text</label>
        <textarea rows={3} value={settings.cancellationPolicy} onChange={e => upd('cancellationPolicy', e.target.value)}
          ...
          className="w-full bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm placeholder-gray-600 focus:outline-none focus:border-brand-gold resize-none" />
        <p className="text-crm-muted mt-1 text-base md:text-lg">Displayed on the booking portal and confirmation emails</p>
      </div>
