      ...
      <div className="bg-crm-surface rounded-xl p-6 w-full max-w-md border border-crm-border shadow-sm shadow-2xl relative text-left max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        {!isLoaded ? (
            ...
            <div className="text-center py-6 bg-crm-surface rounded-lg border border-crm-border shadow-sm flex flex-col items-center">
                <p className="text-crm-muted mb-4 text-[13px]">Please sign in or create an account to book an appointment.</p>
            ...
            <div className="space-y-6">
                {error && <p className="text-status-cancelled bg-status-cancelled/20 p-3 rounded text-[13px]">{error}</p>}
                ...
                {userRole && (
                    <div className="bg-status-info/20 border border-status-info/30 p-4 rounded-lg space-y-3">
                        <div className="flex items-center space-x-2">
                            ...
                            <label htmlFor="walkInToggle" className="font-semibold text-status-info cursor-pointer text-[13px]">I am booking on behalf of a client</label>
                        </div>
                        {isWalkIn && (
                            <div className="space-y-3 pt-2">
                                ...
                                {!selectedExistingClient ? (
                                  <>
                                    <div className="relative">
                                      <label className="block text-crm-muted mb-1 text-[13px]">Search Existing Clients</label>
                                      <input
                                        type="text"
                                        placeholder="Type name or email..."
                                        value={clientSearchQuery}
                                        onChange={(e) => setClientSearchQuery(e.target.value)}
                                        className="w-full bg-crm-surface border border-crm-border shadow-sm rounded p-2 text-[13px] text-crm-text focus:outline-none focus:border-brand-gold"
                                      />
                                        ...
                                        <div className="absolute z-20 w-full mt-1 bg-crm-surface border border-crm-border shadow-sm rounded-lg shadow-xl max-h-40 overflow-y-auto">
                                          {clientSearchResults.map(c => (
                                            <button key={c.id} onClick={() => handleSelectExistingClient(c)} className="w-full text-left px-3 py-2 hover:bg-crm-surface transition-colors border-b border-crm-border last:border-0">
                                              <span className="text-crm-text text-[13px] font-medium">{c.name || 'No name'}</span>
                                              <span className="text-crm-muted text-[11px] ml-2">{c.email}</span>
                                            </button>
                                          ))}
                                        </div>
                                      )}
                                    </div>
                                    <div className="relative flex items-center gap-2 text-[11px] text-crm-muted">
                                  ...
                                  <div className="flex items-center justify-between bg-status-confirmed/20 border border-status-confirmed/30 rounded p-2">
                                    <div>
                                      <span className="text-status-confirmed text-[13px] font-semibold">{selectedExistingClient.name}</span>
                                      <span className="text-crm-muted text-[11px] ml-2">{selectedExistingClient.email}</span>
                                    </div>
                                    <button onClick={handleClearSelectedClient} className="text-crm-muted hover:text-crm-text text-[11px] underline">Change</button>
                                  </div>
                                )}
                                ...
                                {!selectedExistingClient && (
                                  <>
                                    <div>
                                      <label className="block text-crm-muted mb-1 text-[13px]">Client Name *</label>
                                      <input type="text" placeholder="John Doe" value={clientName} onChange={(e) => setClientName(e.target.value)} className="w-full bg-crm-surface border border-crm-border shadow-sm rounded p-2 text-[13px] text-crm-text focus:outline-none focus:border-brand-gold" />
                                    </div>
                                    <div>
                                      <label className="block text-crm-muted mb-1 text-[13px]">Client Email (Optional)</label>
                                      <input type="email" placeholder="john@example.com" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} className="w-full bg-crm-surface border border-crm-border shadow-sm rounded p-2 text-[13px] text-crm-text focus:outline-none focus:border-brand-gold" />
                                    </div>
                                    <div>
                                      <label className="block text-crm-muted mb-1 text-[13px]">Client Phone (Optional)</label>
                                      <input type="tel" placeholder="+1 555-123-4567" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} className="w-full bg-crm-surface border border-crm-border shadow-sm rounded p-2 text-[13px] text-crm-text focus:outline-none focus:border-brand-gold" />
                                    </div>
                                  </>
                                )}
                            </div>
                ...
                {/* ──── Step 1: Date ──── */}
                <div>
                    <label className="block text-crm-muted mb-2 text-[13px]">1. Select Date</label>
                ...
                {/* ──── Step 2: Time ──── */}
                <div>
                    <label className="block text-crm-muted mb-2 text-[13px]">2. Select Time</label>
                    {isLoading ? (
                      <div className="flex items-center gap-2 text-crm-muted text-[13px] py-3">
                        ...
                        <select
                          ...
                          className="w-full border border-crm-border shadow-sm rounded p-3 text-crm-text text-[13px] focus:outline-none focus:border-brand-gold cursor-pointer"
                        >
                        ...
                        <p className="text-status-pending bg-amber-900/20 p-3 rounded text-[13px]">No available slots on this date.</p>
                    ) : null}
                </div>
                ...
                {selectedTime && (
                    <div>
                        <label className="block text-crm-muted mb-2 text-[13px]">3. Select Staff</label>
                        ...
                        ) : <p className="text-status-pending bg-amber-900/20 p-3 rounded text-[13px]">No staff available at this time.</p>}
                    </div>
                )}
                ...
                {selectedStaff && (
                  <div>
                    <label className="block text-crm-muted mb-2 text-[13px]">4. Special Requests <span className="text-crm-muted">(optional)</span></label>
                    <textarea
                      value={bookingNotes}
                      ...
                      rows={2}
                      maxLength={500}
                      className="w-full bg-crm-surface border border-crm-border shadow-sm rounded p-3 text-[13px] text-crm-text focus:outline-none focus:border-brand-gold resize-none placeholder-gray-600"
                    />
                  </div>
                )}
