    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1">
          ...
          <input type="month" value={month} onChange={e => setMonth(e.target.value)} style={inputStyle}
            className="w-full border border-botanical-border rounded p-2.5 text-sm focus:outline-none focus:border-brand-gold [&::-webkit-calendar-picker-indicator]:invert" />
        </div>
        <div className="bg-botanical-surface backdrop-blur-xl shadow-2xl rounded-2xl border border-botanical-border flex-1 relative overflow-hidden group hover:bg-botanical-surface transition-all duration-300 z-20">
      ...
      {byCategory.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {byCategory.map(c => (
            <div key={c.cat} className="bg-botanical-surface p-3 rounded-lg border border-botanical-border text-center">
      ...
      <form onSubmit={handleAdd} className="bg-botanical-surface p-4 rounded-lg border border-botanical-border mb-6 space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <input type="number" step="0.01" min="0" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Amount" required style={inputStyle}
            className="border border-botanical-border rounded p-2 text-sm focus:outline-none focus:border-brand-gold" />
          ...
          <input type="date" value={date} onChange={e => setDate(e.target.value)} style={inputStyle}
            className="border border-botanical-border rounded p-2 text-sm focus:outline-none focus:border-brand-gold [&::-webkit-calendar-picker-indicator]:invert" />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {expenses.map(exp => (
            <div key={exp.id} className="bg-botanical-surface p-3 rounded-lg border border-botanical-border flex justify-between items-center">
