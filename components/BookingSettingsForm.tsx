...
export default function BookingSettingsForm({ shopId }: { shopId: string }) {
  ...
  return (
    <div className="bg-slate-800/60 border border-white/5 rounded-xl p-6 space-y-5">
      <h3 className="text-lg font-bold text-white">📅 Online Booking Settings</h3>
      {msg && <div className="p-3 bg-green-900/30 border border-green-500/30 text-green-300 rounded-lg text-sm">{msg}</div>}

      <label className="flex items-center justify-between p-4 bg-black/20 rounded-lg cursor-pointer">
        <div>
          <p className="text-sm font-medium text-white">Enable Online Booking</p>
          <p className="text-xs text-gray-500">Allow clients to book through your public portal</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="text-sm text-gray-400 block mb-1">Min Advance (hours)</label>
        ...
        <div>
          <label className="text-sm text-gray-400 block mb-1">Max Advance (days)</label>
        ...
        <div>
          <label className="text-sm text-gray-400 block mb-1">Buffer Between Appts (mins)</label>
      ...
      <div>
        <label className="text-sm text-gray-400 block mb-1">Cancellation Window (hours)</label>
        <input type="number" min={0} max={168} value={settings.cancellationWindowHours} onChange={e => upd('cancellationWindowHours', +e.target.value)}
          className="w-full md:w-48 bg-black/40 border border-white/10 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-brand-gold" />
        <p className="text-xs text-gray-600 mt-1">Clients cannot cancel within this window before their appointment</p>
      </div>

      <div>
        <label className="text-sm text-gray-400 block mb-1">Cancellation Policy Text</label>
        <textarea rows={3} value={settings.cancellationPolicy} onChange={e => upd('cancellationPolicy', e.target.value)}
          ...
          className="w-full bg-black/40 border border-white/10 rounded px-3 py-2 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-brand-gold resize-none" />
