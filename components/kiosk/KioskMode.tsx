    ...
    return (
        <main className="h-[100dvh] overflow-y-auto overflow-x-hidden bg-crm-bg">
            ...
            <div className="text-center w-full max-w-7xl mx-auto flex-shrink-0 mb-6 md:mb-10 mt-4">
                <h1 className="font-serif font-bold text-crm-accent mb-2 text-4xl md:text-5xl lg:text-6xl">
                    {userProfile?.shop?.name || "Shop Hub"}
                </h1>
                <h2 className="text-crm-muted text-3xl md:text-4xl">Time & Attendance Kiosk</h2>
            </div>
            ...
            <div className="flex-grow w-full max-w-7xl mx-auto flex flex-col justify-center pb-8 px-4">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 h-full min-h-[600px]">
                    ...
                    <div className="lg:col-span-5 bg-crm-surface p-6 md:p-8 lg:p-12 rounded-3xl border border-crm-border shadow-sm shadow-2xl flex flex-col items-center justify-center relative overflow-hidden group">
                        ...
                        <div className="bg-crm-primary/10 text-crm-accent p-4 rounded-full mb-6 z-10 border border-brand-gold/20 hover:opacity-90">
                            <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path>
                            </svg>
                        </div>
                        <h3 className="font-bold text-crm-text mb-3 text-center z-10 text-2xl md:text-3xl">Scan ID</h3>
                        <p className="text-crm-muted mb-8 md:mb-12 text-center z-10 max-w-xs text-base md:text-lg">Staff members, please scan your personal QR code to clock in or out.</p>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6 lg:gap-8">
                        ...
                        <div className="flex-1 bg-crm-surface p-6 md:p-8 rounded-3xl border border-crm-border shadow-sm shadow-2xl flex flex-col max-h-[50vh] lg:max-h-[350px]">
                            <div className="flex items-center gap-4 mb-6 shrink-0">
                                ...
                                <h3 className="font-serif text-crm-text text-2xl md:text-3xl">Currently Clocked In</h3>
                                        ...
                                        {activeLogs.map(log => (
                                            <div key={log.id} className="bg-crm-surface p-4 rounded-xl flex flex-wrap justify-between gap-x-2 gap-y-2 items-center gap-3 border border-crm-border shadow-sm hover:border-crm-border transition-colors">
                                                <div className="min-w-0">
                                                    <p className="font-bold text-crm-text truncate text-base md:text-lg">{log.user.name || log.user.email.split('@')[0]}</p>
                                                    <p className="text-crm-muted truncate text-base md:text-lg">{log.user.email}</p>
                                                </div>
                                                <div className="text-right shrink-0 bg-crm-surface px-3 py-1.5 rounded-lg border border-crm-border shadow-sm">
                                                    <p className="text-crm-muted uppercase tracking-wider mb-0.5 text-base md:text-lg">In since</p>
                        ...
                        <div className="flex-1 bg-crm-surface p-6 md:p-8 rounded-3xl border border-crm-border shadow-sm shadow-2xl flex flex-col max-h-[50vh] lg:max-h-[350px]">
                            <div className="flex items-center gap-4 mb-6 shrink-0">
                                ...
                                <h3 className="font-serif text-crm-text text-2xl md:text-3xl">Checked-In Clients</h3>
                                <span className="ml-auto bg-crm-accent/20 text-crm-accent px-4 py-1.5 rounded-full text-sm font-bold border border-crm-accent/30">
                                        ...
                                        {waitlist.map(entry => (
                                            <div key={entry.id} className={`p-4 rounded-xl flex justify-between items-center gap-3 border ${entry.status === 'SERVING' ? 'bg-status-confirmed/20 border-status-confirmed/30' : 'bg-crm-surface border-crm-border'}`}>
                                                <div className="flex items-center gap-4 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-bold text-crm-text truncate text-base md:text-lg">{entry.clientName}</p>
                                                        <p className="text-crm-muted text-base md:text-lg">Arrived {new Date(entry.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                                    </div>
                                                </div>
