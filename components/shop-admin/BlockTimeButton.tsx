...
export default function BlockTimeButton({ shopId, staffId, date, time }: { shopId: string, staffId: string, date: string, time: string }) {
  ...
  return (
    <button 
      onClick={handleBlock} 
      disabled={loading}
      className="ml-2 px-2 py-1 text-[10px] bg-botanical-surface hover:bg-slate-600 text-botanical-muted rounded border border-botanical-border uppercase font-bold tracking-wider disabled:opacity-50 transition-colors"
