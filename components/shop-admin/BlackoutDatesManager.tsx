...
export default function BlackoutDatesManager({ shopId }: { shopId: string }) {
  ...
  return (
    <div className="bg-crm-surface border border-crm-border shadow-sm rounded-xl p-6 space-y-5">
      <h3 className="font-bold text-crm-text text-2xl md:text-3xl">🚫 Blackout Dates & Holidays</h3>
      <p className="text-crm-muted text-base md:text-lg">Mark days when your shop is closed. Online booking will be disabled for these dates.</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input type="date" value={newDate} onChange={e => setNewDate(e.target.value)}
          ...
          className="bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm focus:outline-none focus:border-brand-gold flex-shrink-0" />
        ...
        <button onClick={addDate} disabled={!newDate || adding}
          className="px-4 py-2 bg-crm-primary text-white rounded-lg text-sm font-bold disabled:opacity-50 flex-shrink-0 hover:opacity-90">
          {adding ? 'Adding…' : '+ Add Date'}
        </button>
      </div>

      {loading && <div className="text-crm-muted animate-pulse text-sm">Loading…</div>}

      {/* Upcoming dates */}
      {upcoming.length > 0 && (
        <div>
          <p className="text-crm-muted uppercase tracking-widest mb-2 text-base md:text-lg">Upcoming ({upcoming.length})</p>
              ...
              return (
                <div key={d.id} className="flex items-center justify-between p-3 bg-crm-surface rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-crm-text font-medium text-base md:text-lg">{fmt(d.date)}</p>
                      <p className="text-crm-muted text-base md:text-lg">{dayName}{d.reason ? ` · ${d.reason}` : ''}</p>
                    </div>
                  </div>
      ...
      {upcoming.length === 0 && !loading && (
        <p className="text-crm-muted italic text-center py-4 text-base md:text-lg">No upcoming blackout dates. Add holidays or closures above.</p>
      )}
      ...
      {past.length > 0 && (
        <details className="text-sm">
          ...
          <div className="mt-2 space-y-1">
            {past.map(d => (
              <div key={d.id} className="flex items-center justify-between px-3 py-2 text-crm-muted text-xs">
