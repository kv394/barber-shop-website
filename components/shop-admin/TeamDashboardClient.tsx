          ...
          return (
            <div key={staffMember.id} className={`bg-crm-surface border border-crm-border shadow-sm rounded-2xl p-5 flex flex-col shadow-lg transition-all duration-200 hover:shadow-xl hover:border-crm-border ${isOnLeave ? 'ring-1 ring-status-cancelled/50' : ''}`}>
                  ...
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <div>
                      <h2 className="font-bold text-crm-text mb-0.5 flex items-center gap-2 text-xl font-bold">
                        ...
                        {staffMember.role === 'BOOTH_RENTER' && <span className="text-[11px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded uppercase tracking-wider font-bold hover:opacity-90">Booth Renter</span>}
                      </h2>
              ...
              {staffMember.role === 'SHOP_ADMIN' ? (
                <div className="flex-grow bg-crm-surface/50 rounded-xl p-3 mb-4 min-h-[120px] flex flex-col justify-center items-center text-center border border-dashed border-crm-border">
                  <span className="text-3xl mb-2">🛡️</span>
                  <p className="font-semibold text-crm-text text-[13px]">Administrator Account</p>
                  <p className="text-crm-muted text-[11px] mt-1">Virtual role, no client booking schedule.</p>
                </div>
              ) : (
