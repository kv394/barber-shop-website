  ...
  return (
    <>
      <div className="flex justify-end mb-4">
        <Link href={`/shop/${shopId}/portfolio`} className="bg-white text-slate-900 border border-botanical-border shadow-sm px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors font-bold uppercase tracking-wider flex items-center gap-2 shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {initialStaff.map((staffMember: any) => {
          ...
          return (
            <div key={staffMember.id} className={`bg-white border border-botanical-border shadow-sm rounded-2xl p-5 flex flex-col shadow-lg transition-all duration-200 hover:shadow-xl hover:border-gray-200 ${isOnLeave ? 'ring-1 ring-red-500/50' : ''}`}>
              ...
              <div className="flex flex-wrap justify-between gap-x-2 gap-y-2 items-start mb-4">
                <StaffProfileModalWrapper staff={staffMember}>
                  <div className="flex items-center gap-3 text-left">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-white border border-botanical-border shadow-sm flex items-center justify-center shrink-0">
                ...
                <div className="flex flex-col items-end gap-2">
                  {isOnLeave ? (
                    ...
                    <span className="text-sm bg-white text-slate-900 border border-botanical-border shadow-sm px-2.5 py-1 rounded-full font-bold uppercase tracking-wider">Day Off</span>
                    ...
                    <form action={updateDayHoursAction} className="w-full space-y-2 border-t border-gray-200 pt-3">
                        ...
                        <div className="flex justify-center gap-2 items-center">
                            <input 
                                ...
                                className="bg-white text-slate-900 text-xs p-1 rounded border border-botanical-border shadow-sm outline-none focus:ring-1 focus:ring-botanical-primary"
                            />
              ...
              <div className="flex flex-col gap-3 mt-auto pt-4">
                <Link
                    ...
                    className="w-full text-center bg-white hover:bg-gray-50 text-slate-900 text-xs font-semibold py-2.5 rounded-lg transition-colors border border-botanical-border shadow-sm flex items-center justify-center"
                >
                    Edit Schedule
                </Link>
                {!isOnLeave && (
                    <form action={addLeaveAction} className="flex flex-col gap-2">
                        ...
                        <div className="flex justify-center gap-1">
                             <input 
                                ...
                                value={getTimeValue(staffMember.id, 'open', defaultOpen)}
                                onChange={(e) => handleTimeChange(staffMember.id, 'open', e.target.value)}
                                className="w-full bg-white text-slate-600 text-sm p-1 rounded border border-botanical-border shadow-sm outline-none focus:ring-1 focus:ring-botanical-primary"
                            />
                             <input 
                                type="time" 
                                name="endTime" 
