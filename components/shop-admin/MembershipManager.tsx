...
export default function MembershipManager({ shopId }: { shopId: string }) {
  ...
  if (loading) return <div className="animate-pulse text-gray-500 py-4">Loading memberships…</div>;

  return (
    <div className="bg-slate-800/60 border border-white/5 rounded-xl p-6">
      <h3 className="text-lg font-bold text-white mb-2">⭐ Membership Tiers</h3>
      ...
      <form onSubmit={addTier} className="mb-8 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="block text-xs text-gray-400 mb-1">Description / Perks</label>
            <textarea 
              value={description} 
      ...
      <h4 className="text-white font-medium mb-3">Active Membership Tiers</h4>
      {tiers.length === 0 ? (
        <p className="text-gray-500 text-sm py-2">No memberships created yet.</p>
      ) : (
        <div className="space-y-3">
          {tiers.map(t => (
            <div key={t.id} className="p-4 bg-black/20 rounded-lg border border-white/5 flex justify-between items-start gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">{t.description || 'No description provided.'}</p>
              </div>
              <button 
                onClick={() => remove(t.id)} 
