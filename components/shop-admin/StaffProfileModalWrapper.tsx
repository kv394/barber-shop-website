...
export default function StaffProfileModalWrapper({ staff, children }: { staff: any, children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-crm-surface backdrop-blur-sm p-4" onClick={() => setIsOpen(false)}>
          <div className="bg-crm-bg border border-crm-border shadow-sm rounded-2xl p-6 w-full max-w-sm shadow-2xl relative" onClick={e => e.stopPropagation()}>
