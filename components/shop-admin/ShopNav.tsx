...
export function ShopNav({ shopId, userRole, activeTab }: { shopId: string, userRole: string, activeTab: string }) {
  ...
  const mainTabClass = (tabName: string, groupTabs?: string[]) => {
    ...
    return isActive
      ? "px-5 py-2.5 text-sm sm:text-base text-white bg-botanical-primary whitespace-nowrap font-black rounded-xl shadow-[inset_0_4px_8px_rgba(0,0,0,0.3)] transform translate-y-0.5 border border-botanical-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-botanical-primary"
