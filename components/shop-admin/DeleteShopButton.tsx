...
export default function DeleteShopButton({ shopId, shopName, onSuccess }: { shopId: string, shopName: string, onSuccess: () => void }) {
  ...
  return (
    <button 
      ...
      disabled={isDeleting}
      className="bg-status-cancelled/20 text-status-cancelled hover:bg-status-cancelled hover:text-crm-text px-4 py-2 rounded-md text-[13px] transition-colors disabled:opacity-50"
    >
