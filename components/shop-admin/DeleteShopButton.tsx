...
export default function DeleteShopButton({ shopId, shopName, onSuccess }: { shopId: string, shopName: string, onSuccess: () => void }) {
  ...
  return (
    <button 
      ...
      disabled={isDeleting}
      className="bg-red-900/50 text-status-cancelled hover:bg-status-cancelled hover:text-botanical-text px-4 py-2 rounded-md text-sm transition-colors disabled:opacity-50"
    >
