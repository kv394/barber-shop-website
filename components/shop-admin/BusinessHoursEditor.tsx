        ...
        {DAYS.map(day => {
          ...
          return (
            <div key={day} className={`flex items-center gap-3 p-3 rounded-lg transition border border-white/5 ${open ? 'bg-white/5' : 'bg-black/20 opacity-60'}`}>
              ...
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform pointer-events-none ${open ? 'translate-x-5' : 'translate-x-0'}`} />
              </button>
