...
export default function NotificationSettingsForm({ shopId }: { shopId: string }) {
  ...
  useEffect(() => { fetch(`/api/shops/${shopId}/notification-settings`).then(r => r.json()).then(setS); }, [shopId]);
  ...
  if (!s) return <div className="animate-pulse text-gray-500 py-4">Loading…</div>;
  ...
  const Toggle = ({ label, desc, k }: { label: string; desc: string; k: string }) => (
    <label className="flex items-center justify-between p-4 bg-black/20 rounded-lg cursor-pointer hover:bg-black/30 transition">
      ...
      <input type="checkbox" checked={!!s[k]} onChange={e => upd(k, e.target.checked)} className="w-5 h-5 accent-brand-gold" />
    </label>
  );
    ...
    <div className="bg-slate-800/60 border border-white/5 rounded-xl p-6 space-y-4">
      ...
      <div className="space-y-2">
        ...
        {s.appointmentReminders && (
          <div className="ml-4 flex gap-3 flex-wrap">
            <label className="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
              <input type="checkbox" checked={!!s.reminder24h} onChange={e => upd('reminder24h', e.target.checked)} className="accent-brand-gold" /> 24 hours before
            </label>
            <label className="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
              <input type="checkbox" checked={!!s.reminder1h} onChange={e => upd('reminder1h', e.target.checked)} className="accent-brand-gold" /> 1 hour before
            </label>
          </div>
        )}
        ...
        {s.reviewRequests && (
          <div className="ml-4 flex items-center gap-2">
            ...
            <input type="number" min={1} max={48} value={s.reviewRequestDelayH} onChange={e => upd('reviewRequestDelayH', +e.target.value)}
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <select value={s.channel} onChange={e => upd('channel', e.target.value)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="email" value={s.adminEmail} onChange={e => upd('adminEmail', e.target.value)} placeholder="admin@yourshop.com"
          ...
          <div>
            ...
            <input type="tel" value={s.adminPhone} onChange={e => upd('adminPhone', e.target.value)} placeholder="+1 (404) 555-0100"
              className="w-full bg-black/40 border border-white/10 rounded px-3 py-2 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-brand-gold" />
          </div>
        </div>
      </div>
