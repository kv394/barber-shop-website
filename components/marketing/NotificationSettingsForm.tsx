...
export default function NotificationSettingsForm({ shopId }: { shopId: string }) {
  ...
  const Toggle = ({ label, desc, k }: { label: string; desc: string; k: string }) => (
    <label className="flex items-center justify-between p-4 bg-crm-surface rounded-lg cursor-pointer hover:bg-crm-surface transition text-sm">
      <div><p className="font-medium text-crm-text text-base md:text-lg">{label}</p><p className="text-crm-muted text-base md:text-lg">{desc}</p></div>
  ...
  return (
    <div className="bg-crm-surface border border-crm-border shadow-sm rounded-xl p-6 space-y-4">
      <h3 className="font-bold text-crm-text text-2xl md:text-3xl">🔔 Notification Settings</h3>
      <p className="text-crm-muted text-base md:text-lg">Configure automated messages sent to clients and staff.</p>
      ...
      <div className="space-y-2">
        <p className="text-crm-muted uppercase tracking-widest font-semibold pt-2 text-xs">Client Notifications</p>
      ...
      {settings.appointmentReminders && (
          <div className="ml-4 flex gap-3 flex-wrap">
            <label className="flex items-center gap-2 text-crm-muted cursor-pointer text-sm">
      ...
      {settings.reviewRequests && (
          <div className="ml-4 flex items-center gap-2">
            <span className="text-sm text-crm-muted">Send</span>
          <input type="number" min={1} max={48} value={settings.reviewRequestDelayH} onChange={e => updateSetting('reviewRequestDelayH', +e.target.value)}
              className="w-16 bg-crm-surface border border-crm-border shadow-sm rounded px-2 py-1 text-crm-text text-sm focus:outline-none" />
            <span className="text-sm text-crm-muted">hours after appointment</span>
          </div>
        )}
      ...
      <div className="space-y-2">
        <p className="text-crm-muted uppercase tracking-widest font-semibold pt-2 text-xs">Admin Alerts</p>
      ...
      <div className="space-y-3">
        <p className="text-crm-muted uppercase tracking-widest font-semibold pt-2 text-xs">Delivery Channel</p>
        <div>
          <label className="text-crm-muted block mb-1 text-sm">Default Channel</label>
        <select value={settings.channel} onChange={e => updateSetting('channel', e.target.value)}
            className="w-full md:w-48 bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm focus:outline-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-crm-muted block mb-1 text-sm">Admin Alert Email</label>
          <input type="email" value={settings.adminEmail} onChange={e => updateSetting('adminEmail', e.target.value)} placeholder="admin@yourshop.com"
              className="w-full bg-crm-surface border border-crm-border shadow-sm rounded px-3 py-2 text-crm-text text-sm placeholder-gray-600 focus:outline-none focus:border-brand-gold" />
          </div>
          <div>
            <label className="text-crm-muted block mb-1 text-sm">Admin Alert Phone (SMS)</label>
