        ...
        <div className="bg-slate-900/80 backdrop-blur-xl shadow-2xl rounded-2xl border border-white/10 flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-white/10 relative z-20 overflow-hidden transform sm:-translate-y-6 sm:-mx-2 mb-2 sm:mb-6">
          <div className="flex-1 p-5 sm:p-6 relative overflow-hidden group hover:bg-white/5 transition-all duration-300 min-w-0">
            ...
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest font-semibold truncate">Total Referrals</h3>
              <span className="text-blue-500 text-sm">👥</span>
            </div>
            <p className="text-2xl sm:text-3xl font-black text-white break-words leading-tight">{stats.total}</p>
          </div>
          <div className="flex-1 p-5 sm:p-6 relative overflow-hidden group hover:bg-white/5 transition-all duration-300 min-w-0">
            ...
            <p className="text-2xl sm:text-3xl font-black text-white break-words leading-tight">{stats.completed}</p>
          </div>
          <div className="flex-1 p-5 sm:p-6 relative overflow-hidden group hover:bg-white/5 transition-all duration-300 min-w-0">
            <div className="absolute top-0 left-0 w-full h-1 bg-amber-500/80"></div>
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest font-semibold truncate">Pending</h3>
              <span className="text-amber-500 text-sm">⏳</span>
            </div>
            <p className="text-2xl sm:text-3xl font-black text-white break-words leading-tight">{stats.pending}</p>
          </div>
        </div>
      )}
