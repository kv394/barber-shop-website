...
const oldFnStart = "function CustomPageContent({ content, shop, themeColor, className, onBookClick, reviews = [], templateType = 'modern' }: { content: string, shop: any, themeColor?: string, className?: string, onBookClick?: (service: any) => void, reviews?: any[], templateType?: string }) {";
const oldFnRegex = new RegExp(oldFnStart.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[\\s\\S]*?(?=export default function ClientPage)', 'g');
    ...
    <div className={className}>
      {parts.map((part, index) => {
        if (part.toLowerCase() === '\\$\\{products\\}') {
          ...
          return <div key={index} className="max-w-4xl mx-auto" dangerouslySetInnerHTML={{ __html: rawHtml }} />;
        }
      })}
    </div>
  );
...
// Wrap custom pages content rendering to constrain text width in all templates except when inside CustomPageContent.
// The CustomPageContent itself is wrapped in `<CustomPageContent className="prose ...">`. Let's fix the container sizes.
const styleReplacements = [
  { old: 'className="max-w-4xl mx-auto px-6 py-24 min-h-[50vh]"', new: 'className="w-full max-w-5xl mx-auto px-6 py-24 min-h-[50vh]"' }, // minimal
  { old: 'className="max-w-4xl mx-auto px-8 py-32 min-h-[60vh]"', new: 'className="w-full max-w-6xl mx-auto px-8 py-32 min-h-[60vh]"' }, // classic
  { old: 'className="max-w-3xl mx-auto min-h-[60vh]"', new: 'className="w-full max-w-5xl mx-auto px-6 min-h-[60vh]"' }, // noir
  { old: 'className="max-w-4xl mx-auto min-h-[60vh]"', new: 'className="w-full max-w-5xl mx-auto px-6 min-h-[60vh]"' }, // sunset
  { old: 'className="py-32 px-8 min-h-[70vh]"', new: 'className="py-32 px-8 min-h-[70vh] w-full max-w-5xl mx-auto"' }, // editorial section wrapper
  { old: 'className="max-w-4xl mx-auto"', new: 'className="w-full"' }, // editorial inner
  { old: 'className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-[60vh]"', new: 'className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 min-h-[60vh]"' }, // sporty
  { old: 'className="bg-white p-8 md:p-12 rounded-3xl shadow-lg border border-gray-100 overflow-hidden"', new: 'className="bg-white p-8 md:p-12 rounded-3xl shadow-lg border border-gray-100 overflow-hidden w-full max-w-6xl mx-auto"' }, // modern wrapper
  { old: 'className="prose prose-lg max-w-none text-gray-600 font-light leading-relaxed mx-auto"', new: 'className="prose prose-lg max-w-4xl mx-auto text-gray-600 font-light leading-relaxed w-full"' } // minimal prose wrap
];

for (const rep of styleReplacements) {
  content = content.replace(new RegExp(rep.old.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), rep.new);
}

fs.writeFileSync(file, content);
console.log('Fixed CustomPageContent layouts and component sizing');
