...
// 1. Add import
if (!content.includes('InteractiveReviewsSection')) {
  const importStatement = "import InteractiveReviewsSection from '@/components/reviews/InteractiveReviewsSection';\n";
  // Insert right after the dynamic imports
  content = content.replace("import { usePathname } from 'next/navigation';", "import { usePathname } from 'next/navigation';\n" + importStatement);
}

// 2. Rewrite CustomPageContent to handle ${reviews}
const oldFunctionStart = 'function CustomPageContent({ content, shop, themeColor, className, onBookClick }: { content: string, shop: any, themeColor?: string, className?: string, onBookClick?: (service: any) => void }) {';
const oldFunctionRegex = new RegExp(oldFunctionStart.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g, '\\\\$&') + '[\\\\s\\\\S]*?(?=export default function ClientPage)', 'g');

const newFunction = `function CustomPageContent({ content, shop, themeColor, className, onBookClick, reviews = [] }: { content: string, shop: any, themeColor?: string, className?: string, onBookClick?: (service: any) => void, reviews?: any[] }) {
...
// 3. Ensure we pass reviews={reviews} to CustomPageContent
// We'll replace all instances of <CustomPageContent ... onBookClick={handleBookClick} /> to also have reviews={reviews}
...
// 4. Remove all occurrences of <ReviewsSection
const oldReviewsSection = /<ReviewsSection reviews=\{reviews\}[^>]*\/>/g;
content = content.replace(oldReviewsSection, '');

fs.writeFileSync(file, content);
console.log('Added interactive reviews support');