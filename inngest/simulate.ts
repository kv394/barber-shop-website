...
export const simulateLiveShop = inngest.createFunction(
  ...
  async ({ step }) => {
    // 1. Find the Heritage Haircuts demo shop
    const adminUser = await step.run('find-demo-shop', async () => {
    ...
    if (!adminUser || !adminUser.shopId) {
      return { message: 'Demo shop not found.' };
    }
    ...
    const walkInsGenerated = await step.run('generate-walk-ins', async () => {
      ...
      if (!clients.length || !staff.length || !services.length) return 0;
    ...
    const clockIns = await step.run('simulate-clock-ins', async () => {
      ...
      const activeStaffIds = [...new Set(activeAppointments.map(a => a.staffId))];
    ...
    return { lifecycleResult, walkInsGenerated, clockIns };
  }
);
