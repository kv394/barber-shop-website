...
- **Read Replicas**: Set up database Read-Replicas. Send all write requests (booking creation, clocking in) to the primary node, and re-route read-heavy dashboards (like fetching reporting graphs or loading superadmin log tables) to the secondary node to keep the system breathing smoothly under bursts.
