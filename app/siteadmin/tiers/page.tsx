...
export default function SiteAdminTiersPage() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-crm-muted">Loading pricing tiers...</div>;
  }
    ...
    <div>
      <div className="flex flex-wrap justify-between gap-x-2 gap-y-2 items-center mb-8">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="bg-crm-primary text-white px-6 py-2.5 rounded-lg font-bold hover:bg-crm-surface hover:text-crm-primary border border-transparent hover:border-crm-primary/30 transition-colors disabled:opacity-50"
        >
      ...
      <div className="space-y-6">
        {tiers.map((tier, index) => (
          <div key={tier.id} className="bg-crm-surface p-6 rounded-xl border border-crm-border shadow-sm">
            <div className="flex gap-4 items-center mb-4 pb-4 border-b border-crm-border">
              <input
                type="text"
                value={tier.name}
