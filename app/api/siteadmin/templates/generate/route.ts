    ...
    for (const file of files) {
      ...
      if (file.name.endsWith('.zip')) {
        ...
        for (const zipEntry of zipEntries) {
          ...
          if (entryName.endsWith('.html') || entryName.endsWith('.htm')) {
            ...
            uploadedCss = entryBuffer.toString('utf-8');
             ...
             if (fileId) {
               uploadedAssets.push({ fileName: entryName, url: `https://drive.google.com/uc?export=view&id=${fileId}` });
             }
          }
        }
      } else {
        if (file.name.endsWith('.html') || file.name.endsWith('.htm')) {
          ...
          if (fileId) {
            uploadedAssets.push({ fileName: file.name, url: `https://drive.google.com/uc?export=view&id=${fileId}` });
          }
        }
      }
    }
...
{{#each shop.services}}
  <div class="service-card-example ...">
     ...
     <label for="service-{{this.id}}" className="text-[13px]">
       <h3 className="text-lg font-bold">{{this.name}}</h3>
       <p className="text-[13px]">{{this.description}}</p>
...
{{#each shop.users}}
  <div class="staff-card-example ...">
     <h3 className="text-lg font-bold">{{this.name}}</h3>
     <p className="text-[13px]">{{this.role}}</p>
  </div>
{{/each}}
...
{{#each shop.reviews}}
  <div class="review-card-example ...">
     <p className="text-[13px]">"{{this.comment}}"</p>
