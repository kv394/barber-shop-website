...
export async function POST(
 ...
 const { shopId } = await params;
  const authRes = await requireShopRole(shopId, ['SITE_ADMIN', 'SHOP_ADMIN', 'STAFF']);
  ...
  return NextResponse.json({ error: 'Invalid file type. Only images are allowed (no SVG).' }, { status: 400 });
 }
