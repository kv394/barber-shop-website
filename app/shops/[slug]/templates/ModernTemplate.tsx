...
export default function ModernTemplate({ ctx }: { ctx: any }) {
 ...
 return (
  <main className="min-h-screen overflow-x-hidden flex flex-col bg-crm-bg text-crm-text relative">
   {faviconUrl && (
   ...
   {logoUrl ? (
   <div className="w-28 h-28 md:w-36 md:h-36 mb-8 rounded-full overflow-hidden border-4 shadow-2xl bg-crm-surface flex items-center justify-center p-2" style={{ borderColor: primaryColor || '#ffffff' }}>
   ...
   {pages.filter((p: any) => p.isVisible).map((p: any) => (
   <section key={p.id} id={p.id} className="bg-crm-surface p-8 md:p-12 rounded-3xl shadow-lg border border-crm-border overflow-hidden w-full max-w-7xl mx-auto">
   <h2 className="font-extrabold mb-10 text-2xl md:text-3xl text-crm-text text-center tracking-tight" style={{ color: primaryColor }}>{p.title}</h2>
   <CustomPageContent content={p.content || ""} shop={shop} themeColor={primaryColor} className="prose prose-lg max-w-none text-crm-muted mx-auto" onBookClick={handleBookClick} reviews={reviews} templateType={templateType} />
   </section>
   ))}
   </div>
   {/* Footer */}
   <footer className="bg-crm-surface border-t border-crm-border pt-16 pb-8">
   <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:px-8">
   <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12 text-center md:text-left">
   <div>
   <h3 className="font-bold text-crm-text mb-6 text-xl">{shop.name}</h3>
   <p className="text-crm-muted mb-6 text-[14px] leading-relaxed max-w-xs mx-auto md:mx-0">
   {shop.description || 'Your trusted service provider.'}
   </p>
   </div>
   
   <div>
   <h4 className="font-bold text-crm-text mb-6 text-sm uppercase tracking-widest">Contact Info</h4>
   <ul className="space-y-4 text-crm-muted text-[14px]">
   {shopAddress && (
   ...
   <span className="text-lg">📞</span>
   <a href={`tel:${shopPhone}`} className="hover:text-crm-text transition-colors">{shopPhone}</a>
   </li>
   )}
   {shopEmail && (
   <li className="flex items-center justify-center md:justify-start gap-3">
   <span className="text-lg">✉️</span>
   <a href={`mailto:${shopEmail}`} className="hover:text-crm-text transition-colors">{shopEmail}</a>
   </li>
   )}
   </ul>
   </div>
   ...
   <div>
   <h4 className="font-bold text-crm-text mb-6 text-sm uppercase tracking-widest">Follow Us</h4>
   ...
   <div className="border-t border-crm-border pt-8 text-center text-gray-400 text-[13px] font-medium tracking-wide">
