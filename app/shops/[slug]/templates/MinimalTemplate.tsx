...
export default function MinimalTemplate({ ctx }: { ctx: any }) {
 ...
 <a href="#" className="transition-opacity hover:opacity-60 text-crm-muted">Home</a>
 {pages.filter((p: any) => p.isVisible).map((p: any) => (
 <a key={p.id} href={`#${p.id}`} className="transition-opacity hover:opacity-60 text-crm-muted">{p.title}</a>
 ))}
 </nav>
 )}
