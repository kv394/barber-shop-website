...
export default function CorporateTemplate({ ctx }: { ctx: any }) {
 ...
 return (
  <main className="min-h-screen overflow-x-hidden flex flex-col bg-crm-bg text-crm-text relative">
   ...
   {logoUrl ? <Image src={logoUrl} alt={shop.name} /> : <h1 className="font-bold shrink-0 mr-6 text-2xl font-bold" style={{ color: primaryColor }}>{shop.name}</h1>}
   {pages.filter((p: any) => p.isVisible).length > 0 && (
   <nav className="hidden md:flex gap-4 sm:gap-6 shrink-0 mr-6">
   <a href="#" className="text-sm py-2 px-3 font-bold transition-colors text-crm-muted hover:text-crm-text">Home</a>
   {pages.filter((p: any) => p.isVisible).map((p: any) => (
   <a key={p.id} href={`#${p.id}`} className="text-sm py-2 px-3 font-bold transition-colors text-crm-muted hover:text-crm-text">{p.title}</a>
   ))}
   </nav>
   )}
   <MobileNav
