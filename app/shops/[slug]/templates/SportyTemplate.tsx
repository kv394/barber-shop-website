 ...
 return (
  <main className="min-h-screen overflow-x-hidden flex flex-col bg-crm-surface text-crm-text relative">
   ...
   {shop.name}
    ...
    <nav className="hidden md:flex gap-4 sm:gap-6 shrink-0">
    <a href="#" className="text-sm py-2 px-3 font-bold uppercase transition-colors text-crm-muted hover:text-crm-text">Home</a>
    {pages.filter((p: any) => p.isVisible).map((p: any) => (
    <a key={p.id} href={`#${p.id}`} className="text-sm py-2 px-3 font-bold uppercase transition-colors text-crm-muted hover:text-crm-text">{p.title}</a>
    ))}
    </nav>
    )}
    <MobileNav
