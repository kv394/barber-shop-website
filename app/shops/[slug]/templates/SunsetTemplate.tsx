...
export default function SunsetTemplate({ ctx }: { ctx: any }) {
 ...
 return (
  <main className="min-h-screen overflow-x-hidden flex flex-col bg-gradient-to-br from-purple-900 via-black to-orange-900 text-crm-text relative">
   {faviconUrl && (
   ...
   `}} />
    ...
    <nav className="hidden md:flex gap-4 sm:gap-6 bg-crm-surface px-4 sm:px-6 py-2 rounded-full backdrop-blur-md border border-status-pending/30 overflow-x-auto max-w-[calc(100vw-100px)] hide-scrollbar">
    <a href="#" className="text-sm py-2 px-3 font-medium transition-colors whitespace-nowrap text-crm-muted hover:text-status-pending">Home</a>
    {pages.filter((p: any) => p.isVisible).map((p: any) => (
    <a key={p.id} href={`#${p.id}`} className="text-sm py-2 px-3 font-medium transition-colors whitespace-nowrap text-crm-muted hover:text-status-pending">{p.title}</a>
    ))}
    </nav>
    ) : (
    <div />
   ...
   {logoUrl ? <Image src={logoUrl} alt={shop.name} /> : <h1 className="font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-400 to-purple-400 mb-4 text-4xl font-bold">{shop.name}</h1>}
