 ...
 return (
  <main className="min-h-screen overflow-x-hidden flex flex-col bg-crm-surface text-crm-text font-serif relative">
   ...
   `}} />
    ...
    <nav className="hidden md:flex flex-wrap gap-4 md:gap-6 text-sm uppercase tracking-[0.2em]">
    <a href="#" className="py-2 px-3 transition-colors text-crm-muted hover:text-crm-text">Home</a>
    {pages.filter((p: any) => p.isVisible).map((p: any) => (
    <a key={p.id} href={`#${p.id}`} className="py-2 px-3 transition-colors text-crm-muted hover:text-crm-text">{p.title}</a>
    ))}
    </nav>
    )}
    <MobileNav
   ...
   {logoUrl ? <Image src={logoUrl} alt={shop.name} /> : <h1 className="font-black uppercase tracking-tighter text-2xl font-bold">{shop.name}</h1>}
   ...
   <CustomPageContent content={p.content || ""} shop={shop} className="prose prose-invert prose-lg max-w-none text-crm-muted " onBookClick={handleBookClick} reviews={reviews} templateType={templateType} />
   </section>
   ))}
   </div>
   {selectedService && (
