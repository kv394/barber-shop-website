...
export default function TemplatesPage() {
  ...
  return (
    <div className="p-6 text-botanical-text">
      ...
      <div className="bg-botanical-surface p-6 rounded-xl border border-botanical-border mb-8">
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          <div>
            ...
            <input required value={name} onChange={e => setName(e.target.value)} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" />
          </div>
          <div>
            <label className="block text-sm text-botanical-muted mb-1">Description</label>
            <input value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" />
          </div>
          <div>
            <label className="block text-sm text-botanical-muted mb-1">Prompt for Gemini</label>
            <textarea required value={prompt} onChange={e => setPrompt(e.target.value)} rows={4} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" placeholder="Describe the layout, colors, elements, styling... Handlebars syntax will be used for injection." />
          </div>
          <div>
            <label className="block text-sm text-botanical-muted mb-1">AI Model</label>
            <select value={model} onChange={e => setModel(e.target.value)} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text">
      ...
      {loading ? <p className="text-botanical-muted">Loading templates...</p> : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {templates.map((t: any) => (
            <div key={t.id} className="bg-botanical-surface p-4 rounded-xl border border-botanical-border flex flex-col">
      ...
      {editingTemplate && (
        <div className="fixed inset-0 bg-botanical-surface flex items-center justify-center p-4 z-50">
          <div className="bg-botanical-surface border border-botanical-border p-6 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required value={editingTemplate.name} onChange={e => setEditingTemplate({...editingTemplate, name: e.target.value})} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" />
                </div>
                <div>
                  <label className="block text-sm text-botanical-muted mb-1">Description</label>
                  <input value={editingTemplate.description || ''} onChange={e => setEditingTemplate({...editingTemplate, description: e.target.value})} className="w-full bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={10} 
                  className="w-full font-mono text-sm bg-botanical-surface border border-botanical-border p-2 rounded text-botanical-text" 
                />
              </div>
