...
const ROLES = ['ALL', 'SUPER_ADMIN', 'SHOP_ADMIN', 'STAFF', 'CLIENT', 'ATTENDANCE_KIOSK'] as const;
...
export default function SuperAdminUsersPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-serif font-bold text-brand-gold mb-2">User Management</h1>
        <p className="text-gray-400">Manage all users across the platform. {users.length} user{users.length !== 1 ? 's' : ''} found.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <form onSubmit={handleSearch} className="flex-1 flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-800 border border-white/10 rounded-lg px-4 py-2.5 text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-brand-gold"
          />
          <button
            type="submit"
            className="bg-brand-gold text-brand-dark px-6 py-2.5 rounded-lg font-semibold text-sm hover:bg-white transition-colors"
          >
            Search
          </button>
        </form>
        ...
        <div className="bg-slate-800/50 rounded-xl border border-white/10 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gray-400 text-left border-b border-white/10 bg-slate-800/80">
              ...
              <tbody className="divide-y divide-white/5">
                {filteredUsers.map(user => {
                  const roleColors: Record<string, string> = {
                    ...
                    SHOP_ADMIN: 'bg-brand-gold/20 text-brand-gold border-brand-gold/30',
                    ...
                    <tr key={user.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-4 py-3">
                        {editingUser === user.id ? (
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold border ${roleColors[user.role] || 'bg-white/10 text-gray-300'}`}>
                            {user.role.replace(/_/g, ' ')}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-gray-300 text-xs">
                        {user.shopName || <span className="text-gray-500">—</span>}
                      </td>
