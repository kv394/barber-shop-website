...
export default async function ResourcesSettingsPage({ params }: { params: Promise<{ shopId: string }> }) {
  ...
  return (
    <ShopAdminLayout shopName={data.shop.name} shopSlug={data.shopSlug} pageTitle="Resource Management" tabs={data.userRole === 'SITE_ADMIN' ? undefined : settingsTabs} shopId={shopId} userRole={data.userRole} activeTab="settings-resources">
