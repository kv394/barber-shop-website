    ...
    <ShopAdminLayout shopName={shop.name} shopSlug={shopSlug} pageTitle={userRole === 'SITE_ADMIN' ? 'Assign Shop Admin & Kiosk' : 'Team & Availability'} shopId={shopId} userRole={userRole} activeTab="team">
      ...
      {kioskUser && (
         <div className="bg-blue-50 border border-blue-200 p-5 rounded-2xl shadow-lg mb-8">
             ...
             <p className="text-botanical-muted mb-4 text-base md:text-lg">To set up the front desk attendance tablet, sign up for a new account on that device using this exact email:</p>
             <div className="bg-botanical-surface p-3 rounded-lg text-center mb-3 border border-botanical-border shadow-sm"><code className="text-botanical-primary font-mono font-bold tracking-wider">{kioskUser.email}</code></div>
      ...
      <div className="mb-8 bg-botanical-surface rounded-2xl border border-botanical-border shadow-sm shadow-xl overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 rounded-xl bg-botanical-primary/10 flex items-center justify-center text-xl hover:opacity-90">✉️</div>
            <div>
              <h3 className="font-bold text-botanical-text text-2xl md:text-3xl">Invite Team Member</h3>
              <p className="text-botanical-muted text-base md:text-lg">Add staff or admins to your shop</p>
            </div>
          </div>
          ...
          <form action={inviteUser} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div className="md:col-span-6">
              <label className="block text-botanical-muted mb-1 font-semibold uppercase tracking-wider text-sm">📧 Email</label>
              <input type="email" name="email" required placeholder="team@example.com"
                className="w-full p-2.5 rounded-lg border border-botanical-border shadow-sm bg-botanical-surface text-botanical-text text-sm placeholder-gray-400 focus:ring-2 focus:ring-botanical-primary focus:outline-none transition-all" />
            </div>
            <div className="md:col-span-3">
              <label className="block text-botanical-muted mb-1 font-semibold uppercase tracking-wider text-sm">👤 Role</label>
              <select name="role" defaultValue={userRole === 'SITE_ADMIN' ? 'SHOP_ADMIN' : 'STAFF'}
                className="w-full p-2.5 rounded-lg border border-botanical-border shadow-sm bg-botanical-surface text-botanical-text text-sm focus:ring-2 focus:ring-botanical-primary focus:outline-none transition-all">
      ...
      {userRole === 'SITE_ADMIN' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {staff.map((staffMember: any) => (
            <div key={staffMember.id} className="bg-botanical-surface border border-botanical-border shadow-sm rounded-2xl p-5 flex flex-col shadow-lg">
              <div className="flex flex-wrap justify-between gap-x-2 gap-y-2 items-start mb-4">
                <div>
                  <h2 className="font-bold text-botanical-text mb-1 flex items-center gap-2 text-3xl md:text-4xl">
                    {staffMember.name || staffMember.email.split('@')[0]}
                    <span className={`text-xs px-1.5 py-0.5 rounded uppercase tracking-wider font-bold ${staffMember.role === 'SHOP_ADMIN' ? 'bg-botanical-primary/10 text-botanical-primary border border-botanical-primary/20' : 'bg-blue-50 text-blue-700 border border-blue-200'} hover:opacity-90`}>
          ...
          {staff.length === 0 && (
            <p className="text-botanical-muted italic text-base md:text-lg">No assigned admins or kiosks.</p>
          )}
        </div>
      ) : (
