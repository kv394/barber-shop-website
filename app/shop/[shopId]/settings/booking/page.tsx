...
export default async function BookingSettingsPage({ params }: { params: Promise<{ shopId: string }> }) {
  ...
  return (
    <ShopAdminLayout shopName={data.shop.name} shopSlug={data.shopSlug} pageTitle="Booking & Hours" tabs={data.userRole === 'SITE_ADMIN' ? undefined : settingsTabs} shopId={shopId} userRole={data.userRole} activeTab="settings-booking">
