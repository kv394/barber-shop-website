...
export default async function BookingSettingsPage({ params }: { params: Promise<{ shopId: string }> }) {
  ...
  return (
    <ShopAdminLayout shopName={data.shop.name} shopSlug={data.shopSlug} pageTitle="Booking & Hours" shopId={shopId} userRole={data.userRole} activeTab="settings">
      <SettingsSubNav shopId={shopId} active="booking" />
