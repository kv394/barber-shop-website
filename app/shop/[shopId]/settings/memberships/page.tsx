...
export default async function MembershipsSettingsPage({ params }: { params: Promise<{ shopId: string }> }) {
  ...
  return (
    <ShopAdminLayout shopName={data.shop.name} shopSlug={data.shopSlug} pageTitle="Membership Tiers" tabs={data.userRole === 'SITE_ADMIN' ? undefined : settingsTabs} shopId={shopId} userRole={data.userRole} activeTab="settings-memberships">
