...
async function main() {
  try {
    ...
    if (!shop) {
      console.log('Heritage Haircuts shop not found. Proceeding to create template only.');
    }
    ...
    if (shop) {
      ...
      console.log(`Successfully updated shop ${shop.name} to use the heritage-haircuts-template!`);
    }
  } catch (error: any) {
    console.error("Database connection failed, but script has been created:", error.message);
  }
}
