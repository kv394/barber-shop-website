...
import { uploadFileToPath, getOrCreateFolder, downloadFileFromFolder } from '../lib/google-drive';
import fs from 'fs';

async function main() {
  const targetShopId = 'heritage-haircuts';
  const name = 'black-theme2';
  const description = 'Black themed template generated via script';
  const prompt = 'A clean, dark-mode, neon green futuristic barbershop layout';
  const zipPath = '/Users/rajaveerappan/Downloads/stitch_beauty_salon_landing_page-3.zip';
  ...
  for (const zipEntry of zipEntries) {
    ...
    if (entryName.endsWith('.html') || entryName.endsWith('.htm')) {
      ...
      uploadedCss = entryBuffer.toString('utf-8');
       ...
       console.log(`Uploading ${entryName} to Drive...`);
...
------------------------------
`;
  let shopName = 'Heritage Haircuts';
...
HTML:
\`\`\`html
${baseHtml.substring(0, 30000)}
\`\`\`

CSS:
\`\`\`css
${baseCss.substring(0, 10000)}
\`\`\`
-----------------------`;
  }
...
-----------------------`;
  ...
  console.log('Skipping DB save in script...');
  const template = { id: 'script-generated-' + Date.now() };

  console.log('Uploading generated index.html and styles.css to Drive...');
  ...
  console.log('Done! Template ID:', template.id);
  
  // Validate Google Drive logic
  const bFolder = await getOrCreateFolder('barbersaas');
  const shopFolder = await getOrCreateFolder(targetShopId, bFolder || undefined);
  const themeFolder = await getOrCreateFolder(name, shopFolder || undefined);
  
  // Now list files to show what exists
  console.log('Listing files in drive folder:', themeFolder);
  if (themeFolder) {
    const html = await downloadFileFromFolder(themeFolder, 'index.html');
    if (html) {
      console.log('Successfully fetched index.html from Google Drive!');
      console.log('--- PREVIEW (FIRST 1000 CHARS) ---');
      console.log(html.substring(0, 1000));
    } else {
      console.log('Failed to fetch index.html from Google Drive.');
    }
  }
}
