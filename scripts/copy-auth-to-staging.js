...
async function run() {
  const sourceUrl = 'postgresql://postgres.mfzeljdiepffgdqioiqp:geqrom-gubgy8-zIkdog@aws-1-us-east-2.pooler.supabase.com:5432/postgres'; // Prod
  const targetUrl = 'postgresql://postgres.yjavfwugdkpghszspnrw:gadJur-tydvoj-8wymza@aws-1-us-east-1.pooler.supabase.com:5432/postgres'; // Staging
  ...
  try {
    ...
    // Fetch from source (Prod)
    const res = await sourceClient.query("SELECT * FROM auth.users WHERE email IN ('admin@heritagehaircuts.com', 'admin@luxurynails.com')");
    ...
    const resId = await sourceClient.query("SELECT * FROM auth.identities WHERE user_id IN (SELECT id FROM auth.users WHERE email IN ('admin@heritagehaircuts.com', 'admin@luxurynails.com'))");
